...
**Product direction (see README "Product direction"):** the wedge is recurring small friend groups/clubs, not one-off parties or work polls. When choosing what to build, prefer in order: (1) frictionless no-account guest RSVP/vote/comment via the invite link, (2) transactional email notifications, (3) calendar import auto-blocking availability + ranking times, (4) recurring/series events with a group home, (5) intent-based monetization (affiliate on dinner/trip, organizer premium) — never paywall basics. Feature breadth is explicitly deprioritized; instrument the growth funnel (activation, invite→participant, K-factor, W4 retention) in the existing PostHog wiring before adding surface area. Watch costs: Clerk MAU (mitigated by guest flow) and calendar-data trust/scope minimalism. Phase 2 (only after the private loop works): public events discoverable by locality/interest, follow people/topics — needs event `visibility`, topic tags, follow edges, moderation basics (see README).
