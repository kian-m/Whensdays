...
| **Rich link previews** | Any shared invite | Texting an invite link shows a branded card (`og:image` + `summary_large_image`) featuring the **Whensdays logo** plus the event title & time | `handleOGPage` (`ogpage.go`) + `apps/web/public/og-card.png` |
