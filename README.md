...
| **Following** | Group page & event page ("Hosted by" row) | **Follow a host or a group** — an artist, venue or club — and their **listed** events land in your feed. Asymmetric and **not Groups**: you can follow a club without joining it (Follow sits next to Join on the group preview), and members can follow too. Hosts control what surfaces per event with a **"Show to my followers"** checkbox (create wizard + edit form, on by default; every pre-existing event was backfilled OFF). *Deliberately not called "subscriptions" — that word means .ics calendar sync here* | `POST /api/follows` `{kind: host\|topic\|group}`, `DELETE /api/follows/{kind}/{value}`, `GET /api/feed?scope=following`; `events.listed` (migration `0044`) |
