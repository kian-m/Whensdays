...
| **Group invite links** | Group page | Any member shares the group link (or its QR) - anyone who opens it, including no-account guests, previews the group and joins in one tap, then sees all its events | `GET /api/groups/{id}/preview`, `POST /api/groups/{id}/join` |
