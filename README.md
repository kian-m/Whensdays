...
| **Following** | Group page & event page ("Hosted by" row) | **Follow a host or a group** — an artist, venue or club — and their **listed** events land in your feed. Asymmetric and **not Groups**: you can follow a club without joining it (Follow sits next to Join on the group preview), and members can follow too. Hosts control what surfaces per event with a **"Show to my followers"** checkbox (create wizard + edit form, on by default; every pre-existing event was backfilled OFF). *Deliberately not called "subscriptions" — that word means .ics calendar sync here*. Followers also get a **once-daily digest email** of new listed events from the pages they follow (created in the last 24h; skips events they created or already RSVP'd to) — one email per recipient with cover thumb, one-tap RSVP, and a per-page "unfollow" link, riding the existing daily reminders cron with no schedule of its own | `POST /api/follows` `{kind: host\|topic\|group}`, `DELETE /api/follows/{kind}/{value}`, `GET /api/feed?scope=following`; `events.listed` (migration `0044`); digest: `followdigest.go`, `GET /api/follows/unsubscribe?token=` (HMAC, unauthenticated) |
