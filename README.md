...
| **Multi-date finalize** | General poll results (host) | Read the group's availability, then schedule **several winning dates at once** — they become one series with everyone (RSVPs intact) on each date | `more_starts` on finalize; attendees + invites copied per occurrence |
