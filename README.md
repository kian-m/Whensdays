...
| **Start a plan (no account)** | Free scheduler (`/free-scheduler`) → "Start a plan" | A name is all it takes: guest identity → the create flow (title + time, or an availability poll) → share the link. `/start` renders the same single create flow as `/new`. The static page is a one-pager with a live "N events scheduled so far" counter (any1.free-style, hidden until a real count loads) | `POST /api/guest/join` without an `event_id`; `GET /api/public/stats` (unauthenticated, count of locked-in events) |
