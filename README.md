...
| **Add** ✅ | Finalize celebration + shareable card | Shipped: lock-moment banner + a canvas-rendered 1080x1080 card ("It's happening" + title/date in the event's theme accent) on every scheduled event - native share / download; group streaks get the same card ("N months strong"). The invite unfurl now carries **"N in so far"** (og.png + description) so the group chat sees momentum |
...
| **Private calendar import** | Profile → Connected calendars | Google (OAuth), **Apple (CalDAV + app-specific password - nothing published)**, **Outlook (Microsoft OAuth)**, or a published link as fallback; all read-only, credentials encrypted at rest | `POST /api/calendar/apple-caldav`, `GET /api/calendar/outlook/connect` |
...
| **Group streaks** | Group page + email | "🔥 N-month streak" — consecutive months with at least one event; a shareable "N months strong" card, and when the streak extends into a new month **every member gets one congratulation email** (once per group per month) | web: computed from the group's events; email: daily cron, `group_streak_congrats` gate |
