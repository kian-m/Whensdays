...
| **Buzz Mill venue sync** | Daily (cron) | The group's Buzz Mill (buzzmillcoffee.com, an Austin bar with a long-running weekly stand-up program) comedy nights track the venue's real schedule from its **Squarespace JSON feed** (any page + `?format=json`) — no scraper or browser, the server fetches it directly: `POST /api/cron/buzzmill-sync` (X-Cron-Key, body `{group_id}`) filters to **comedy** (case-insensitive "comedy" in the title — chosen empirically: the feed mixes trivia/DJ sets/a *music* open mic/park cleanups, so "open mic" would be exactly the wrong keyword) and, being a curated single-venue list, **auto-creates** any comedy series the group lacks (title, cleaned description, dates, poster pulled in as the cover), then keeps every series current (add/retime/quiet-cancel) hosted by a hidden **Buzz Mill Schedule bot** with the group owner kept on as cohost. Same FLAT Squarespace feed shape + engine as We Improv, but Austin (Central time, `America/Chicago`); the tracked nights are booked stand-up showcases (mapped to the `openmic` = "open mic / showcase night" preset) — no Austin stand-up *open mic* was reachable via a clean feed | `apps/api/buzzmillsync.go` + `venuesync.go`; secrets `CRON_KEY`, group id |
...
| **Email notifications** | Automatic | Uses your **account email** (from Clerk — no separate field to fill): hosts hear about RSVPs & comments in **~10-minute digests** (one email even when people comment repeatedly or flip RSVPs); attendees get an **invite**, a **time-locked** note, a **day-before reminder** (sent 2pm Pacific for next-day events), and a **cancel** notice. Branded HTML (logo, the **event's theme colors**, and its **cover GIF** when set); a person with **several events tomorrow gets one digest email** listing them all; **times render in the event's timezone** (the host's, captured at creation), not UTC; every link is UTM-tagged so PostHog attributes email-driven visits. No-op unless `EMAIL_API_KEY`/`EMAIL_FROM` set | `internal/notify` (Resend-compatible) + `emails.go` templates + `notifications.go` triggers, async; `POST /api/cron/reminders` (X-Cron-Key) |
...
| **Best-time ranking** | Poll results (host) | Candidate times are ranked by votes AND how they fit **everyone's** saved availability (🟢 free / 🔴 busy per option) — not just the viewer's calendar | `option_fit` computed server-side from all attendees' availability |
...
| **Group invite links** | Group page | Any member shares the group link (or its QR) - anyone who opens it, including no-account guests, previews the group and joins in one tap, then sees all its events. **Pasting the link into a chat unfurls a rich preview** — the group's GIF/icon as the tile, **"&lt;who&gt; invited you to join"**, and the group name — just like an event invite | `GET /api/groups/{id}/preview`, `POST /api/groups/{id}/join`; unfurl via `handleGroupOGPage` + `GET /api/groups/{id}/og.png` (bounces browsers to `/gv/{id}`) |
